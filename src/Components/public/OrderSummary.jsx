...
const OrderSummary = ({ plan, simType = "esim" }) => {
  ...
  return (
    <aside className="lg:sticky lg:top-28 lg:self-start">
      ...
      <div className="relative mt-4 rounded-2xl border border-white/10 bg-secondary p-5 sm:p-6">
        {plan.popular && (
          <span className="absolute right-5 top-5 rounded-full bg-btnPrimary px-2.5 py-0.5 text-[10px] font-bold tracking-wide text-white">
            POPULAR
          </span>
        )}

        <p className="pr-20 text-base font-bold text-white">
        ...
        <div className="mt-3 flex items-end gap-1">
          ...
          <span className="text-[28px] font-bold leading-none text-white sm:text-[34px]">
            {plan.price}
          </span>
        ...
        <div className="mt-6 space-y-2 border-t border-white/10 pt-5">
          ...
          <p className="text-xs text-white/45">Monthly (VAT included)</p>
        </div>
      </div>
    </aside>
  );
