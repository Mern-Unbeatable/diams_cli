...
const CollaboratorEditCustomerModal = ({
  isOpen,
  onClose,
  customer,
  onSave,
  ...
  useEffect(() => {
    if (customer) {
      setFormData({
        ...
        plan: customer.plan || "NovaSky Max",
        simType: customer.simType || "eSIM",
        ...
        <div className="flex items-start justify-between pb-4 border-b border-slate-100">
          <div>
            ...
            <p className="mt-0.5 text-xs text-slate-400">
              Update customer information, plan, and status.
            </p>
          </div>
            ...
            <div>
              ...
              <div className="relative">
                <select
                  ...
                  className="w-full appearance-none rounded-xl border border-slate-200 bg-white py-2.5 pl-3.5 pr-8 text-xs font-medium text-slate-900 outline-none transition-colors hover:border-slate-300 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 cursor-pointer"
                >
                  {statusOptionsWithoutAll.map((opt) => (
                    <option key={opt} value={opt}>
                      {opt}
                    </option>
                  ))}
...
export default CollaboratorEditCustomerModal;
