...
const EsimDetailsView = ({ profile, onBack, onStatusChange }) => {
  ...
  const esimId =
    profile?.esimId || profile?.id?.replace("esim-", "E-509") || "E-5091";
  ...
  return (
    <div className="w-full space-y-6 pb-12 text-slate-900 font-sans">
      ...
      <div>
        ...
        <p className="mt-0.5 font-mono text-xs text-slate-400">{esimId}</p>
      </div>

      {/* 1. Main Profile Card */}
      <div className="rounded-xl border border-slate-100 bg-white p-6 shadow-sm sm:p-8 space-y-6">
      ...
      <div className="rounded-xl border border-sky-100 bg-[#f6fbff] p-5 sm:p-6 space-y-1">
        <h3 className="text-sm font-bold text-slate-900">QR delivery status</h3>
