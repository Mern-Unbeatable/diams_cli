  ...
  return (
    <section className="page-section bg-white">
      <div className="mx-auto flex flex-col gap-6 container px-4 sm:px-6 lg:gap-8 lg:px-10">
        <div className="flex flex-col items-start justify-between gap-5 rounded-2xl border border-red-200 bg-red-50 px-5 py-5 sm:flex-row sm:items-center sm:px-6 sm:py-6">
          <div className="flex items-start gap-4">
            <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-red-100">
              <AlertCircle size={22} className="text-red-600" strokeWidth={1.75} />
            </span>
            <div>
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm sm:p-8">
          <div className="grid gap-8 lg:grid-cols-[1fr_2fr] lg:items-center lg:gap-10">
            ...
            <ul className="grid gap-6 sm:grid-cols-3">
              {items.map(({ id, icon, title: itemTitle, description: itemDesc }) => {
                  ...
                  <li key={id}>
                    <Link to="/help" className="group block">
                      <Icon
                        ...
                        className="text-textsecondary"
                      />
                      <p className="mt-3 font-bold text-primary">{itemTitle}</p>
                      <p className="mt-1 text-sm text-primary/60">{itemDesc}</p>
                    </Link>
                  </li>
                );
              })}
